...
def main():
    ...
    out_dir = os.path.expanduser("~/kvbench_repro/results_100/topk")
    ...
    for context, question, idx in samples:
        ...
        keep_len = max(1, int(original_len * args.ratio))

        input_ids = input_ids[:, :keep_len].cuda()
        attention_mask = attention_mask[:, :keep_len].cuda()
        ...
        result = {
            ...
            "kept_input_tokens": int(keep_len),
            "output_tokens": int(gen_tokens),
            "latency_s": latency_s,
            "peak_vram_mib": peak_vram_mib,
        }
        all_results.append(result)
        print(f"[{args.dataset}][{idx}] ratio={args.ratio} lat={latency_s:.4f} mem={peak_vram_mib:.1f}")

    ratio_str = str(args.ratio).replace(".", "p")
    out_path = os.path.join(out_dir, f"{args.dataset}_topk_100_r{ratio_str}.json")
